...
function drawHeader($s=0){
	...
	<header class='header'>
    <div class='container'>
        <a href='index.php' ><img href='index.php' src='assets/img/Petzee1.png' ></a>";
		if(isLogged() == 1){
			logout();
			myaccount();
			echoaddadslog($s);
...
function echoaddadslog($s){
	...
	<a   class='btn btn-lg btn-block addads ' href='createads.php'>+ SELL YOUR PETS</a></div>";
	}else {
	echo "</header>";
	}
}
...
function echoaddadsnolog(){
	echo "</header>
	<div class='container control'>
	<a   class='btn btn-lg btn-block addads ' href='' data-toggle='modal' data-target='#myModal'>+ SELL YOUR PETS</a>
	</div>
	";
}
